...
const minor = parseInt(React.version.split('.')[1], 10);
...
switch (minor) {
  ...
  case 6:
    ...
    const AnonymousMemoized = React.memo(({label}) => <label>{label}</label>);
    ...
    class Suspending extends React.Component {
      ...
      render() {
        if (this.state.useSuspense) {
          ...
          return (
            <button onClick={this.useSuspense}>
              load data
            </button>
          );
        }
  ...
  case 4:
    ...
    class ProfilerChild extends React.Component {
      ...
      render() {
        return (
          <div>
            count: {this.state.count} <button onClick={this.incrementCount}>increment</button>
          </div>
        );
      }
    }
    ...
    apps.push(
      <Feature key="unstable_Profiler" label="unstable_Profiler" version="16.4+">
  ...
  case 3:
    ...
    apps.push(
      <Feature key="AsyncMode/ConcurrentMode" label="AsyncMode/ConcurrentMode" version="16.3+">
        <ConcurrentMode>
          <div>unstable_AsyncMode was added in 16.3, renamed to unstable_ConcurrentMode in 16.5, and then renamed to ConcurrentMode in 16.7</div>
        </ConcurrentMode>
      </Feature>
    );
  case 2:
  ...
  case 0:
    ...
    class SimpleApp extends React.Component {
      ...
      render() {
        ...
        return (
          <div>
            {count % 2 === 0 ? (
              <span>count: {count} <small>(even)</small></span>
            ) : (
    ...
    apps.push(
      <Feature key="Simple stateful app" label="Simple stateful app" version="16.0">
        <SimpleApp />
      </Feature>
    );
    break;
