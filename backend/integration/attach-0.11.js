...
var group = makeReporter(test.createStream({objectMode: true}));
...
group('Mounting & Unmounting', () => {
  ...
  test('attaching late should work', t => {
    ...
    var node = document.createElement('div');
    React.renderComponent(<SimpleApp/>, node);
  ...
  test('should register two roots', t => {
    ...
    wrapRender(hook, () => {
      ...
      React.renderComponent(<SimpleApp/>, node);
      React.renderComponent(<SimpleApp/>, node2);
      React.unmountComponentAtNode(node);
...
group('State updating', () => {
  test('State update', t => {
    ...
    wrapNode(node => {
      wrapRender(hook, () => {
        var App = React.renderComponent(<StateApp/>, node);
  ...
  test('Props update', t => {
    ...
    wrapNode(node => {
      wrapRender(hook, () => {
        var App = React.renderComponent(<StateProps/>, node);
